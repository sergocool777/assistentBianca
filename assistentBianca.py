...
ndel = ['Бьянка', 'Bianca', 'Бьяночка', 'ладно', 'не могла бы ты','пожалусйта']

commands = ['привет', 'увидимся','открой файл', 'выключи комп', 'выруби компьютер', 'пока', 'покажи файл','покажи список команд',
...
def comparison(x): # осуществляет поиск самой подходящей под запрос функции
	...
	for i in range(len(commands)):
		...
		if (k > 50)&(k > j):
			ans = commands[i]
			j = k
	if (ans != 'пока')& (ans != 'увидимся')& (ans != 'привет'):
		add_file(ans)
	return(str(ans))
...
def musik(): # включает музыку
	webbrowser.open('https://vk.com/audios227633928')
...
cmds = {
	...
	'переведи' : check_translate,               'увидимся' : quit
}
...
but1 = Button(root, text = 'Слушаю', command = main)
